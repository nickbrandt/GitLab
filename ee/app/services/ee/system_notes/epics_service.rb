  ...
  module SystemNotes
    class EpicsService < ::SystemNotes::BaseService
      ...
      def change_epics_relation(child_epic, type)
        ...
        change_epics_relation_act(type, note_body,
                                  { direction: 'child', target_epic_ref: child_epic.to_reference(noteable.group) })

        create_note(NoteSummary.new(child_epic, nil, author, note_body % { direction: 'parent', target_epic_ref: noteable.to_reference(child_epic.group) }, action: type))
      end

      def change_epics_relation_act(action, text, text_params)
        create_note(NoteSummary.new(noteable, nil, author, text % text_params, action: action))
      end
