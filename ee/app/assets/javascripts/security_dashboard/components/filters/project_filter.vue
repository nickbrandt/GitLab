...
export default {
  ...
  computed: {
    options() {
      return Object.values(this.availableProjects);
    },
    selectedSet() {
    ...
    isSearchTooShort() {
      return this.searchTerm.length < SEARCH_TERM_MINIMUM_LENGTH;
    },
    isSearching() {
      return this.searchTerm.length > 0;
    },
    showSelectedProjectsSection() {
      return this.selectedOptions.length && !this.searchTerm.length > 0;
    },
    hasSelectedMaxNumberOfProjects() {
      return this.selectedOptions.length >= SELECTED_PROJECTS_MAX_COUNT;
    },
    querystringIdsWithoutAll() {
      return this.querystringIds.includes(this.filter.allOption.id) ? [] : this.querystringIds;
    },
    unavailableIds() {
      return this.querystringIdsWithoutAll.filter((id) => !this.availableProjects[id]);
    },
  },
  apollo: {
    // This query gets the project names from the project IDs in the querystring.
    projectsById: {
      query: groupProjects,
      manual: true,
      variables() {
        return {
          ...
          ids: this.unavailableIds.map((id) => `${PROJECT_ID_PREFIX}${id}`),
        };
      },
      result({ data }) {
        const projects = mapProjects(data.group.projects.nodes);
        projects.forEach((project) => {
          this.$set(this.availableProjects, project.id, project);
        });
      },
      error() {
      ...
      skip() {
        return !this.unavailableIds.length;
      },
    },
    // This query gets the projects for the group with an optional search.
    projects: {
      query: groupProjects,
      variables() {
        return {
...
<template>
  ...
  >
    ...
    <gl-dropdown-text v-else-if="hasSelectedMaxNumberOfProjects">
      {{ $options.i18n.maxProjectsSelected }}
    </gl-dropdown-text>
    <gl-dropdown-text v-else-if="isSearching && isSearchTooShort">
