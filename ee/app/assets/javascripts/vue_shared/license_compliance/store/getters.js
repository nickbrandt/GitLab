...
import { LICENSE_APPROVAL_STATUS, REPORT_GROUPS_BY_STATUS } from '../constants';
import { groupBy } from 'lodash';

const addLicensesMatchingGroupStatus = licenseReport => {
  const licensesGroupedByStatus = groupBy(licenseReport, 'status');

  return reportGroupByStatus => ({
    ...reportGroupByStatus,
    licenses: licensesGroupedByStatus[reportGroupByStatus.status] || [],
  });
};

const hasAtLeastOneLicense = ({ licenses }) => licenses.length > 0;
...
export const licenseReportGroups = (_, getters) =>
  REPORT_GROUPS_BY_STATUS.map(addLicensesMatchingGroupStatus(getters.licenseReport)).filter(
