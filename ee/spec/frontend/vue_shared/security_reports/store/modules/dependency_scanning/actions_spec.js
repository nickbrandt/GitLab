...
import * as actions from 'ee/vue_shared/security_reports/store/modules/dependency_scanning/actions';
...
describe('dependency scanning report actions', () => {
  beforeEach(() => {
    state = createState();
  });
  ...
  describe('requestReports', () => {
    it(`should commit ${types.REQUEST_REPORTS}`, done => {
      testAction(actions.requestReports, null, state, [{ type: types.REQUEST_REPORTS }], [], done);
    });
  });
