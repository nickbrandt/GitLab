...
describe Evidences::ReleaseEntity do
  ...
  context 'when report artifacts are passed' do
    ...
    let(:build_test_report) { create(:ci_build, :test_reports, pipeline: pipeline, name: 'build_1') }
    let(:build_coverage_report) { create(:ci_build, :coverage_reports, pipeline: pipeline, name: 'build_2') }
