...
describe Releases::CreateEvidenceService do
  ...
  context 'when pipeline with artifacts is passed' do
    ...
    let!(:build_with_artifacts) { create(:ci_build, :artifacts, pipeline: pipeline, name: 'build_1') }
    ...
    it 'includes test reports in evidence if feature is licenced' do
      ...
      service.execute
      evidence_reports = Releases::Evidence.last.summary['release']['report_artifacts'].map {|artifact| artifact['url']}
