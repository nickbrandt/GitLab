...
RSpec.describe GroupWikiPage::Slug do
  let_it_be(:meta) { create(:group_wiki_page_meta) }

  describe 'Associations' do
    it { is_expected.to belong_to(:group_wiki_page_meta) }

    it 'refers correctly to the wiki_page_meta' do
      created = create(:group_wiki_page_slug, group_wiki_page_meta: meta)

      expect(created.reload.group_wiki_page_meta).to eq(meta)
    end
  end
  ...
  describe 'scopes' do
    describe 'canonical' do
      ...
      context 'there is at least one canonical slugs' do
        before do
          create(:group_wiki_page_slug, :canonical)
        end
  ...
  describe 'Validations' do
    ...
    describe 'only_one_slug_can_be_canonical_per_meta_record' do
      ...
      context 'there are other slugs, but they are not canonical' do
        before do
          create(:group_wiki_page_slug, group_wiki_page_meta: meta)
        end
      ...
      context 'there is already a canonical slug' do
        before do
          create(:group_wiki_page_slug, canonical: true, group_wiki_page_meta: meta)
        end
