...
class Ci::FreezePeriodService
  ...
  def execute
    ...
    project.project_deploy_freeze_periods.each do |period|
      previous_freeze_start = previous_time(period.freeze_start, timezone_name(period.timezone))
      previous_freeze_end = previous_time(period.freeze_end, timezone_name(period.timezone))
      next_freeze_start = next_time(period.freeze_start, timezone_name(period.timezone))
      next_freeze_end = next_time(period.freeze_end, timezone_name(period.timezone))
