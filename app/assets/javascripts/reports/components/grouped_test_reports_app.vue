...
export default {
  ...
  computed: {
    ...
    showViewFullReport() {
      return this.glFeatures.junitPipelineView;
    },
  },
  created() {
    ...
    <template v-if="showViewFullReport" #actionButtons>
      <div>
        <a
          :href="testTab"
          target="_blank"
          class="btn btn-default btn-sm float-right append-right-default report-link"
        >
          <span class="report-link-text">{{ s__('ciReport|View full report') }}</span>
