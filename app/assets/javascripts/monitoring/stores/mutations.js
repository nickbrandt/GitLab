 ...
 * @param {String} metricKey Unique id in the dashboard
 * @param {Object} dashboard Full dashboard object
 */
const findMetricInDashboard = (metricKey, dashboard) => {
  let res = null;
...
export default {
   ...
   */
  [types.REQUEST_METRIC_RESULT](state, { metricKey }) {
    const metric = findMetricInDashboard(metricKey, state.dashboard);
    setMetricState(metric, {
      loading: true,
      state: metricStates.LOADING,
    });
  },
  [types.RECEIVE_METRIC_RESULT_SUCCESS](state, { metricKey, result }) {
    if (!metricKey) {
      return;
    }
  ...
  [types.RECEIVE_METRIC_RESULT_FAILURE](state, { metricKey, error }) {
    if (!metricKey) {
      return;
    }
    const metric = findMetricInDashboard(metricKey, state.dashboard);
