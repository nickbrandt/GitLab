...
class MembersFinder
  ...
  def direct_group_members(include_descendants)
    ...
    GroupMembersFinder.new(group).execute(include_relations: requested_relations).non_invite.non_limited_access # rubocop: disable CodeReuse/Finder
  end
  ...
  def project_invited_groups_members
    ...
    GroupMember.with_source_id(invited_groups_ids_including_ancestors).non_limited_access
  end
