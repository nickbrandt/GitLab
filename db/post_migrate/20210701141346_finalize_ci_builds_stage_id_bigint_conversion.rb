...
class FinalizeCiBuildsStageIdBigintConversion < ActiveRecord::Migration[6.1]
  ...
  def swap_columns
    ...
    add_concurrent_index TABLE_NAME, :stage_id_convert_to_bigint, name: :index_ci_builds_on_converted_stage_id
    ...
    with_lock_retries(raise_on_exhaustion: true) do
      ...
      execute "LOCK TABLE #{quoted_table_name}, #{quoted_referenced_table_name} IN ACCESS EXCLUSIVE MODE"

      # Swap the column names of the two columns
      temporary_name = 'stage_id_tmp'
      execute "ALTER TABLE #{quoted_table_name} RENAME COLUMN stage_id TO #{quote_column_name(temporary_name)}"
      ...
      # Swap defaults of the two columns
      change_column_default TABLE_NAME, :stage_id, nil
      change_column_default TABLE_NAME, :stage_id_convert_to_bigint, 0

      # Remove the original column index, and rename the new column index to the original name
      remove_index TABLE_NAME, name: :index_ci_builds_on_stage_id # rubocop:disable Migration/RemoveIndex, Migration/WithLockRetriesDisallowedMethod
