...
class FinalizeCiJobArtifactsBigintConversion < ActiveRecord::Migration[6.1]
  ...
  def swap
    ...
    # This is to replace the existing "index_ci_job_artifacts_id_for_terraform_reports" btree (project_id, id) where (file_type = 18)
    ...
    add_concurrent_index TABLE_NAME, [:job_id_convert_to_bigint, :file_type], name: 'index_ci_job_artifacts_on_job_id_and_file_type_bigint'
    ...
    with_lock_retries(raise_on_exhaustion: true) do
      ...
      id_function_name = Gitlab::Database::UnidirectionalCopyTrigger.on_table(TABLE_NAME).name(:id, :id_convert_to_bigint)
      execute "ALTER FUNCTION #{quote_table_name(id_function_name)} RESET ALL"
      job_id_function_name = Gitlab::Database::UnidirectionalCopyTrigger.on_table(TABLE_NAME).name(:job_id, :job_id_convert_to_bigint)
      execute "ALTER FUNCTION #{quote_table_name(job_id_function_name)} RESET ALL"
