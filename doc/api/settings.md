...
the `file_template_project_id`, `project_deletion_adjourned_period` or the `geo_node_allowed_ips` parameters:
...
- `geo_status_timeout`
- `project_deletion_adjourned_period`
...
| `project_deletion_adjourned_period`      | integer          | no                                   | **(PREMIUM)** How many days after marking project for deletion it is actually removed. Value between 0 and 7.
