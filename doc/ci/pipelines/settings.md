...
A deployment job can fail because a newer one has run. In order to prevent that an environment is
accidentally deployed with an older source code, when clicking on `Retry` a modal will
