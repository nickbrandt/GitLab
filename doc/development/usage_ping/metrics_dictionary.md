...
| `data_source`       | yes      | `string`; may be set to a value like `database`, `redis`, `redis_hll`, `prometheus`, `ruby`, `system`. |
...
#### Metric with `data_source: prometheus`, `ruby` or `system`

For metrics instrumented with Prometheus or Ruby, or coming from the operating system,
