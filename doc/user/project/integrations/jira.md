...
- **Run a pipeline on a MR linked to a Jira issue**:
  - The Jira issue shows the current pipeline status (in the sidebar as "builds")
- **Deploy to an environment from a MR linked to a Jira issue**:
  - The Jira issue shows the status of the deployment (in the sidebar as "deployments")
- **Create or modify a feature flag that names a Jira issue in its description**:
