  ...
  describe '#down' do
    ...
    context 'application on group cluster' do
      it 'removes only redundant and inconsistent entries' do
        migration.down
        expect(services.all.map(&:attributes)).to match_array([manual_inactive_service, active_service_group_application].map(&:attributes))
      end
    end
    ...
    context 'application on instance cluster' do
      it 'removes only redundant and inconsistent entries' do
        ...
        expect(services.all.map(&:attributes)).to match_array([active_service_without_application_on_group_cluster, manual_inactive_service, active_service_group_application].map(&:attributes))
      end
    end
  end
end
