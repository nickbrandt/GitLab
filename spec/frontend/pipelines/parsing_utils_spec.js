...
import { mockParsedGraphQLNodes, nonExistantJob } from './components/dag/mock_data';
...
describe('DAG visualization parsing utilities', () => {
  ...
  describe('makeLinksFromNodes', () => {
    ...
    it('does not generate a link for non-existing jobs', () => {
      ...
      expect(sources.includes((val) => val === nonExistantJob)).toBe(false);
    });
  });
  ...
  describe('removeOrphanNodes', () => {
    it('removes sankey nodes that have no needs and are not needed', () => {
      ...
      */
      expect(parsed.nodes).toHaveLength(22);
