# frozen_string_literal: true
require 'spec_helper'

RSpec.describe WebHooks::ExecuteWorker do
  include AfterNextHelpers

  let_it_be(:project_hook) { create(:project_hook) }
  let_it_be(:data) { { foo: 'bar' }.to_json }
  let_it_be(:hook_name) { 'push_hooks' }

  describe '#perform' do
    it 'delegates to WebHookService' do
      expect_next(WebHookService, project_hook, Gitlab::Json::PrecompiledJson.new(data), hook_name).to receive(:execute)

      subject.perform(project_hook.id, data, hook_name)
    end

    it_behaves_like 'worker with data consistency',
                  described_class,
                  feature_flag: :load_balancing_for_web_hook_worker,
                  data_consistency: :delayed
  end
end
