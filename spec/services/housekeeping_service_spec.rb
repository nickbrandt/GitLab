...
RSpec.describe HousekeepingService do
  ...
  context 'wikis' do
    ...
    let_it_be(:wiki) { project.wiki }

    subject { described_class.new(wiki) }

    before do
      wiki.reset_pushes_since_gc
    end
    ...
    describe '#execute' do
      it 'enqueues a sidekiq job' do
        ...
        expect(subject).to receive(:lease_key).and_return(:the_lease_key)
        expect(subject).to receive(:task).and_return(:incremental_repack)
        expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :incremental_repack, :the_lease_key, :the_uuid).and_call_original
      ...
      it 'resets counter after execution' do
        expect(subject).to receive(:try_obtain_lease).and_return(:the_uuid)
        allow(subject).to receive(:gc_period).and_return(1)
        wiki.increment_pushes_since_gc

        perform_enqueued_jobs do
          expect { subject.execute }.to change { wiki.pushes_since_gc }.to(0)
        end
      end
      ...
      context 'when no lease can be obtained' do
        ...
        it 'does not enqueue a job' do
          ...
          expect { subject.execute }.to raise_error(Wikis::HousekeepingService::LeaseTaken)
        end

        it 'does not reset pushes_since_gc' do
          expect do
            expect { subject.execute }.to raise_error(Wikis::HousekeepingService::LeaseTaken)
          end.not_to change { wiki.pushes_since_gc }
        end

        it 'does not yield' do
          expect do |block|
            expect { subject.execute(&block) }
              .to raise_error(Wikis::HousekeepingService::LeaseTaken)
          end.not_to yield_with_no_args
        end
      end
      ...
      context 'task type' do
        it 'goes through all three housekeeping tasks, executing only the highest task when there is overlap' do
          ...
          # At push 200
          expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :gc, :the_lease_key, :the_uuid)
            .once
          # At push 50, 100, 150
          expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :full_repack, :the_lease_key, :the_uuid)
            .exactly(3).times
          # At push 10, 20, ... (except those above)
          expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :incremental_repack, :the_lease_key, :the_uuid)
            .exactly(16).times
          # At push 6, 12, 18, ... (except those above)
          expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :pack_refs, :the_lease_key, :the_uuid)
            .exactly(27).times
          ...
          expect(wiki.pushes_since_gc).to eq(1)
        end
      end

      it 'runs the task specifically requested' do
        housekeeping = described_class.new(wiki, :gc)

        allow(housekeeping).to receive(:try_obtain_lease).and_return(:gc_uuid)
        allow(housekeeping).to receive(:lease_key).and_return(:gc_lease_key)

        expect(GitGarbageCollectWorker).to receive(:perform_async).with(wiki.id, :gc, :gc_lease_key, :gc_uuid).twice
    ...
    describe '#needed?' do
      ...
      it 'when the count is high enough' do
        allow(wiki).to receive(:pushes_since_gc).and_return(10)
        expect(subject.needed?).to eq(true)
      end
    end
    ...
    describe '#increment!' do
      it 'increments the pushes_since_gc counter' do
        expect { subject.increment! }.to change { wiki.pushes_since_gc }.by(1)
      end
    end
  end
end
