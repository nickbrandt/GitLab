...
RSpec.describe CaseSensitivity do
  describe '.iwhere' do
    ...
    it 'finds a single instance by a single value regardless of case' do
      expect(model.iwhere(path: 'MODEL-1')).to contain_exactly(model_1)
    end

    it 'finds multiple instances by a single value regardless of case' do
      expect(model.iwhere(path: %w(MODEL-1 model-2))).to contain_exactly(model_1, model_2)
    end

    it 'finds instances by multiple values' do
