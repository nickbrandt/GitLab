  ...
  def stub_feature_flags(features)
    features.each do |feature_name, option|
      ...
      if thing
        ...
        allow(Feature).to receive(:disabled?).with(feature_name, thing, any_args) { !enabled }
        allow(Feature).to receive(:disabled?).with(feature_name.to_s, thing, any_args) { !enabled }
      else
        ...
        allow(Feature).to receive(:disabled?).with(feature_name, any_args) { !enabled }
        allow(Feature).to receive(:disabled?).with(feature_name.to_s, any_args) { !enabled }
      end
    end
  end
end
