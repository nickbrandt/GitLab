...
RSpec.shared_examples 'a glob matching rule' do
  ...
  where(:case_name, :globs, :files, :satisfied) do
    ...
    'exact top-level casefold match'   | ['dockerfile']               | { 'Dockerfile' => '', 'Gemfile' => '' }            | true
    'exact top-level no match'         | ['Dockerfile']               | { 'Gemfile' => '' }                                | false
    'pattern top-level match'          | ['Docker*']                  | { 'Dockerfile' => '', 'Gemfile' => '' }            | true
    'pattern top-level casefold match' | ['docker*']                  | { 'Dockerfile' => '', 'Gemfile' => '' }            | true
    ...
    'pattern nested casefold match'    | ['src/**/*.go']              | { 'src/gitlab.com/goproject/goproject.GO' => '' }  | true
