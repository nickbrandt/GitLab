...
shared_examples 'archivable validations' do
  ...
  context 'when entry value is not correct' do
    describe '#errors' do
      context 'when value of attribute is invalid' do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} name should be a string"
        end
      end
      ...
      context 'when there is an unknown key present' do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} config contains unknown keys: test"
        end
      end
      ...
      context "when 'expose_as' is not a string" do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} expose as should be a string"
        end
      end
      ...
      context "when 'expose_as' is too long" do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} expose as is too long (maximum is 100 characters)"
        end
      end
      ...
      context "when 'expose_as' is an empty string" do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} expose as " + Gitlab::Ci::Config::Entry::Artifacts::EXPOSE_AS_ERROR_MESSAGE
        end
      end
      ...
      context "when 'expose_as' is used without 'paths'" do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} paths can't be blank"
        end
      end
      ...
      context "when 'paths' includes '*' and 'expose_as' is defined" do
        ...
        it 'returns error' do
          expect(entry.errors)
            .to include "#{class_name} paths can't contain '*' when used with 'expose_as'"
        end
      end
    end
  end
  ...
  describe 'excluded artifacts' do
    context 'when configuration is valid and the feature is enabled' do
      ...
      context 'when configuration is not valid' do
        ...
        it 'returns an error' do
          ...
          expect(entry.errors)
            .to include "#{class_name} exclude should be an array of strings"
        end
      end
    end
    ...
    context 'when artifacts/exclude feature is disabled' do
      ...
      context 'when configuration has been provided' do
        ...
        it 'returns an error' do
          expect(entry).not_to be_valid
          expect(entry.errors).to include "#{class_name} exclude feature is disabled"
        end
      end
