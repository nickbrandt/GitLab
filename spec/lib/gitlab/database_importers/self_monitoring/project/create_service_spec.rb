...
describe Gitlab::DatabaseImporters::SelfMonitoring::Project::CreateService do
  describe '#execute' do
    ...
    context 'with application settings and admin users' do
      let(:project) { Project.find(result[:project_id]) }
      let(:group) { Group.find(result[:group_id]) }
      let(:application_setting) { Gitlab::CurrentSettings.current_application_settings }
      ...
      it 'returns correct keys' do
        expect(result.keys).to contain_exactly(
          :status, :project_id, :group_id, :project_full_path
        )
      end

      it 'returns project_full_path' do
        expect(result).to include(project_full_path: project.full_path)
      end
