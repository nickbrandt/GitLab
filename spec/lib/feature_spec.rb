...
RSpec.describe Feature, stub_feature_flags: false do
  ...
  describe '.enabled?' do
    ...
    it 'caches the status in L1 and L2 caches', :request_store, :use_clean_rails_memory_store_caching do
      described_class.enable(:enabled_feature_flag)
    ...
    it 'returns the default value when the database does not exist' do
      ...
      expect(described_class.enabled?(:a_feature, default_enabled: true)).to eq(true)
    end
      ...
      it 'caches the status in L2 cache after 2 minutes' do
        Timecop.travel 2.minutes do
          expect do
            ...
            expect(described_class.enabled?(flag)).to be_truthy
          end.not_to exceed_query_limit(1) # TODO: (Figure out): Changed from 0 to 1. There are two calls to fetch list of feature and a feature
        end
      end
