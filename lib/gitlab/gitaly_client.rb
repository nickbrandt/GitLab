  ...
  module GitalyClient
    ...
    def self.stub_certs
      ...
      @certs = stub_cert_paths.flat_map do |cert_file|
        File.read(cert_file).scan(PEM_REGEX).map do |cert| # rubocop:disable Performance/AvoidIoRead
    ...
    def self.filesystem_id_from_disk(storage)
      metadata_file = File.read(storage_metadata_file_path(storage)) # rubocop:disable Performance/AvoidIoRead
